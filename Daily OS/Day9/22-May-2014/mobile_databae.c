...
{
	char mob_num[20];
	printf("Enter the mob_num(10 digit) :");
	scanf("%s",mob_num);
	...
	{
		if(strcmp(mob_num,temp.mob_num)==0)
		{
			flag=1;
		}
		else
		{
			...
			printf("do u wanna add mobile number in database [1] :yes, [2] :no \n");
			scanf("%d", &option);
			if(option==1)
			{
				struct list new_detail;
				printf("Enter the mob_num u wanna add: ");
				scanf("%s",new_detail.mob_num);
				int value=0;
				int flag=0;
				value= validate_mob_num(new_detail.mob_num);
					printf("Enter First name:");
					scanf("%s",new_detail.Firstname);
					printf("Enter Lastname: ");
					scanf(" %s",new_detail.Lastname);
					printf("Enter Mail_id :");
					scanf("%s",new_detail.mail_id);
				flag=validate_mail_id(new_detail.mail_id);
				add_data_in_file(new_detail);
			}
...
void display()
{
	...
	while(fread(&temp,sizeof(temp),1,fp)  ==1)
	{
		printf(" MOB_NUM          FIRSTNAME            LASTNAME          MAIL_ID\n");
		printf("%s\t ",temp.mob_num);
